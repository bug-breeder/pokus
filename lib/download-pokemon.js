#!/usr/bin/env node
/**
 * Pokemon Sprite Copier & Pokedex Generator
 * 
 * Copies Gen 5 (Black/White) sprites from local pokeapi-sprites repo
 * Uses hardcoded Gen 1 Pokemon data (no API calls needed)
 * 
 * Usage: node lib/download-pokemon.js
 */
...
// Configuration
const CONFIG = {
  spriteSource: path.join(__dirname, '../../pokeapi-sprites/sprites/pokemon/versions/generation-v/black-white'),
  spriteDestination: path.join(__dirname, '../assets/raw/pokemon'),
  pokedexOutput: path.join(__dirname, '../utils/pokedex.js'),
}

// Gen 1 Pokemon data (hardcoded - no API needed)
// Type IDs: 1=Normal, 2=Fighting, 3=Flying, 4=Poison, 5=Ground, 6=Rock, 7=Bug, 8=Ghost
...
const GEN1_POKEMON = [
  ...
  { id: 35, name: 'Clefairy', typeId: 1, typeId2: 0 },     // Normal (Fairy in Gen6+)
  { id: 36, name: 'Clefable', typeId: 1, typeId2: 0 },     // Normal (Fairy in Gen6+)
  { id: 37, name: 'Vulpix', typeId: 10, typeId2: 0 },      // Fire
  { id: 38, name: 'Ninetales', typeId: 10, typeId2: 0 },   // Fire
  { id: 39, name: 'Jigglypuff', typeId: 1, typeId2: 0 },   // Normal (Normal/Fairy in Gen6+)
  { id: 40, name: 'Wigglytuff', typeId: 1, typeId2: 0 },   // Normal (Normal/Fairy in Gen6+)
  ...
  { id: 122, name: 'Mr. Mime', typeId: 14, typeId2: 0 },   // Psychic (Psychic/Fairy in Gen6+)
...
function copySprite(id) {
  const srcPath = path.join(CONFIG.spriteSource, `${id}.png`)
  const destPath = path.join(CONFIG.spriteDestination, `${id}.png`)
...
/**
 * Generate pokedex.js content
 */
function generatePokedexFile(pokemonList) {
  const entries = pokemonList.map(p => {
    const t1 = getTypeName(p.typeId)
    const t2 = p.typeId2 ? `/${getTypeName(p.typeId2)}` : ''
    return `  { id: ${String(p.id).padStart(3, ' ')}, name: '${p.name}', typeId: ${String(p.typeId).padStart(2, ' ')}, typeId2: ${String(p.typeId2).padStart(2, ' ')} }, // ${t1}${t2}`
  }).join('\n')
...
export function getPokemonById(id) {
  return POKEDEX.find(p => p.id === id) || null
}
`
}
 ...
 */
function main() {
  console.log('🎮 Pokemon Sprite Copier & Pokedex Generator')
  console.log('============================================\n')
  
  // Check source exists
  if (!fs.existsSync(CONFIG.spriteSource)) {
    console.error(`❌ Sprite source not found: ${CONFIG.spriteSource}`)
    console.error('   Please clone pokeapi-sprites repo:')
    console.error('   git clone https://github.com/PokeAPI/sprites ../pokeapi-sprites')
    process.exit(1)
  }
  
  // Ensure destination exists
  if (!fs.existsSync(CONFIG.spriteDestination)) {
    fs.mkdirSync(CONFIG.spriteDestination, { recursive: true })
  }
  
  console.log(`📦 Copying ${GEN1_POKEMON.length} Pokemon sprites...\n`)
  
  let copied = 0
  let failed = 0
  
  for (const pokemon of GEN1_POKEMON) {
    const ok = copySprite(pokemon.id)
    if (ok) {
      copied++
      process.stdout.write(`  ✅ #${String(pokemon.id).padStart(3, '0')} ${pokemon.name}\n`)
    } else {
      failed++
      process.stdout.write(`  ⚠️  #${String(pokemon.id).padStart(3, '0')} ${pokemon.name} - sprite not found\n`)
    }
  }
  
  console.log(`\n📝 Generating pokedex.js...`)
  const content = generatePokedexFile(GEN1_POKEMON)
  fs.writeFileSync(CONFIG.pokedexOutput, content)
  
  console.log(`\n✅ Done!`)
  console.log(`   Sprites copied: ${copied}`)
  console.log(`   Sprites missing: ${failed}`)
  console.log(`   Pokedex: ${CONFIG.pokedexOutput}`)
}
