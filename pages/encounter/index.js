...
import { onKey, offKey, onGesture, offGesture, KEY_HOME, KEY_SELECT, KEY_SHORTCUT, KEY_EVENT_CLICK, GESTURE_UP } from '@zos/interaction'
import { DEVICE_WIDTH, DEVICE_HEIGHT, COLOR, getGameConfig, getTypeColor } from '../../utils/constants'
...
function isInGreenZone() {
  return state.cursorX >= GREEN_ZONE_START && 
...
function getCatchChance() {
  ...
  if (isInGreenZone()) {
    ...
    return config.CATCH_CHANCE_GREEN_EDGE + 
  ...
  animateThrow(() => {
    animateShake(() => {
      ...
      if (state.headerText) {
        ...
        state.headerText.setProperty(hmUI.prop.COLOR, state.caught ? COLORS.text.success : COLORS.text.danger)
